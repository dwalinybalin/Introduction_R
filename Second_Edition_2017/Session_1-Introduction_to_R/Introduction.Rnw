...
\item Simple and effective programming language (called ``S'').
\end{itemize}
...
\begin{itemize}
\item Distributed as an installer with the name R-X.Y.Z-win.exe (Most updated 3.4.0)
...
\begin{frame}[fragile]\frametitle{What is ``RStudio'' ?}
...
\begin{frame}[fragile]\frametitle{Installing RStudio}

\begin{itemize}
\item The RStudio software under Windows OS is distributed as an installer with the name RStudio-X.Y.Z.exe (Most updated 1.0.143)
\item RStudio page -- \url{https://www.rstudio.com}
...
\begin{itemize}
\item Changing the ``Start in'' field in the R shortcut in windows before opening it.
\item Going to ``File''-->``Change dir'' inside R.
