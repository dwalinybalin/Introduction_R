...
It's important to know where are we woking. There's a default path when starting R that can be shown with the function:
...
\texttt{ls()}: Show the names of all objects in memory.

<<ls2, eval=TRUE>>=
ls() # Show the names of all objects in memory.
@

\texttt{rm()}: Removes one or more objects from memory.
...
Libraries (or packages) are groups of functions that adds new methods or improved functionalities to \texttt{R}. To see which packages are installed the function \texttt{library} can be used, and to see which are already loaded, the \texttt{search} function.
...
Packages can be installed with \texttt{install.packages} function, or if using \texttt{R}, or RStudio, with the interactive menu options.
...
The workspace can be also saved and loaded in the same way as objects, but instead use \texttt{save.image} and the same \texttt{load} function.

<<Saving_Loading2>>=
save.image("Rcourse.RData") # Save all the objects in the workspace to the 'Rcourse.RData' file.
rm(list = ls()) # Remove all the objects in workspace
ls() # list all the objects in workspace
load(file = "Rcourse.RData") # Load all the objects in the workspace from the 'Rcourse.RData' file.
ls()# list all the objects in workspace
@
...
When we have to close the session, we can click the close button of the window, go to the \texttt{File} menu and select quit, or use the q() function. 
...
When working with labelled variables or character strings, we will find the \texttt{character} mode. It is imputed with double quotes ".
...
Matrices are two-dimensional variables of the same mode, while matrices are k-dimensional.
<<matrices>>=
